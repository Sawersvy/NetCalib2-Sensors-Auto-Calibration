...
from calibration_script.calibration_utils import Resampler
import copy

def merge_selected_radar(nusc, sample_idx, modality, frame_range=[0,12],  mis_calibrate_matrix=None):
    '''
    ...
    def current_2_ref(current_pc, current_sd_rec, M_refCam_from_global, ref_time):        
        ...
        raw_pc1 = copy.deepcopy(current_pc)
        current_pc.transform(trans_matrix)
        origin_pc1 = copy.deepcopy(current_pc)
        
        # add noise
        if mis_calibrate_matrix is not None:
            current_pc.transform(mis_calibrate_matrix)
        
        time_lag = abs(time_lag)
        
        return current_pc, trans_matrix, time_lag, origin_pc1, raw_pc1
    ...
    pc = PointCloud.from_file(pcl_path)   
    
    all_raw_pc1 = PointCloud(np.zeros((pc.nbr_dims(), 0)))
    all_origin_pc1 = PointCloud(np.zeros((pc.nbr_dims(), 0)))
    all_pc1 = PointCloud(np.zeros((pc.nbr_dims(), 0)))
    all_pc2 = PointCloud(np.zeros((pc.nbr_dims(), 0)))
    ...
    for _ in range(nsweeps):
        ...
        # point clouds that have not yet undergone any conversion are unified here because of the need to standardize the sampling order.
        current_pc = pc.from_file(osp.join(nusc.dataroot, current_sd_rec['filename']))
        current_pc.remove_close(min_distance)
        # point cloud downsampling
        transformes = transforms.Compose(
                    [Resampler(resample_num)]) # downsampling the point cloud to 100 points requires a fixed dimension for later calculations
        pc_temp = {'points': current_pc.points}
        pc_temp['points'] = pc_temp['points'].T
        current_pc.points = transformes(pc_temp)['points'].T
        
                
        current_pc1, trans_matrix1, time_lag1, origin_pc1, raw_pc1 = current_2_ref(copy.deepcopy(current_pc), current_sd_rec, M_refCam1_from_global, ref_time1)
        current_pc2, _, time_lag2, _, _ = current_2_ref(copy.deepcopy(current_pc), current_sd_rec, M_refCam2_from_global, ref_time2)
        ...
        all_raw_pc1.points = np.hstack((all_raw_pc1.points, raw_pc1.points))
        all_origin_pc1.points = np.hstack((all_origin_pc1.points, origin_pc1.points))
    ...
    if modality == 'radar' :       
        ...
        return x1, y1, depth1, all_times1, x2, y2, depth2, all_times2, rcs, v_comp, all_pc1, all_origin_pc1, all_raw_pc1, trans_matrix1
    ...
    x1, y1, x2, y2, depth1, depth2, all_times1, all_times2 = x1[msk], y1[msk], x2[msk], y2[msk], depth1[msk], depth2[msk], all_times1[msk], all_times2[msk]
                  
    return x1, y1, depth1, all_times1, x2, y2, depth2, all_times2, all_pc1, all_origin_pc1, all_raw_pc1, trans_matrix1
