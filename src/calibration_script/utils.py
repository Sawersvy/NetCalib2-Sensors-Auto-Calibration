...
def inverse_normalize(radar_depth, max_depth):
    try:

        radar_depth[radar_depth>max_depth] = 0  

        radar_indices = torch.where(radar_depth > 0)

        if len(radar_indices[0]) == 0:
            return radar_depth
        
        # radar_max = torch.max(radar_depth[radar_indices])
        # radar_min = torch.min(radar_depth[radar_indices])
        radar_max = max_depth 
        radar_min = 3
        
        i_max = 1/radar_max
        i_min = 1/radar_min
        radar_depth[radar_indices] =1/(radar_depth[radar_indices])
        radar_depth[radar_indices]  = (radar_depth[radar_indices] - i_max) / (i_min - i_max)
        return radar_depth
        ...
        # i_min = 1/radar_min
        # radar_depth[radar_indices] =1/(radar_depth[radar_indices])
        # radar_depth_inverse_normalized = (radar_depth - i_max) / (i_min - i_max)
        # return radar_depth_inverse_normalized
        
    except:
        print(radar_depth, radar_depth.shape, radar_indices)
        assert False
        
def inverse_normalize_batch(radar_depth_batch, max_depth):
    try:
        batch_size = radar_depth_batch.shape[0]
        normalized_batch = torch.zeros_like(radar_depth_batch)
        for i in range(batch_size):
            normalized_batch[i] = inverse_normalize(radar_depth_batch[i], max_depth)
